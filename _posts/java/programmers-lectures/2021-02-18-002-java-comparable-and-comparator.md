...
- 자바에서 같은 타입의 인스턴스를 서로 비교해야만 하는 클래스들은 모두 `Comparable` 인터페이스를 구현하고 있다.
  - 따라서, **`Boolean`을 제외한 래퍼 클래스**나 `String, Time, Date`와 같은 클래스의 인스턴스는 모두 정렬 가능.
...
- `Friend` 클래스에 나이 오름차순 정렬 기준을 정하는 `Comparable` 인터페이스를 구현하면 다음과 같다.
  ...
  public int compareTo(Friend o) {
    // 현재 객체가 크면 클수록 더 큰 양수가 반환되어, 뒤 순서에 위치하게 된다.
    return this.age - o.age;
  }
  ```
  - `Friend` 클래스 내부에 정해진 정렬 규칙(나이 오름차순)으로 배열이 정렬이 되는 것을 볼 수 있다. 만약, 나이 내림차순으로 정렬하려면 다음과 같이 작성하면 된다.
  
    ```java
    @Override
    public int compareTo(Friend o) {
      // 현재 객체가 크면 클수록 더 큰 음수가 반환되어, 앞 순서에 위치하게 된다.
      return o.age - this.age;
    }
    ```
  
- 🔔🔔🔔🔔🔔🔔Friend 객체를 List로 다루는 경우 정렬🔔🔔🔔🔔🔔🔔 
